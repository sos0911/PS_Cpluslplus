...
int main(){
	...
	for(int i=0;i<xs;i++)
		for(int j=0;j<ys;j++){
			...
			int a=psum[i+1][j+1];
			maxv=max(maxv,a);
			int b=psum[xs][j+1]-psum[i+1][j+1]-psum[xs][0]+psum[i+1][0];
			maxv=max(maxv,b);
			int c=psum[i+1][ys]-psum[i+1][j+1]-psum[0][ys]+psum[0][j+1];
			maxv=max(maxv,c);
			int d=psum[xs][ys]-psum[xs][j+1]-psum[i+1][ys]+psum[i+1][j+1];
			maxv=max(maxv,d);
			ans=min(ans,maxv);
		}
	printf("%d\n", ans);
