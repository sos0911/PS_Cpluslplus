...
int main(){
	...
	while(!bfsq.empty()){
		for(int i=0;i<bfsq.size();i++){
			...
			bfsq.pop();
			cout<<"poped : "<<here.cen.fi<<" "<<here.cen.se<<" : "<<here.cen.ver<<'\n';
			...
			for(int j=0;j<4;j++){
				...
				int nextx=here.cen.se+dir[j][1];		if(isrange(nexty,nextx,here.ver)&&check2(nexty,nextx,here.ver)&&!visit[nexty][nextx][here.ver]){
